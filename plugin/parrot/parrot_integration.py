...
PARROT_HOME = Path(os.path.dirname(os.path.realpath(__file__)))
print("PARRot home", PARROT_HOME)
...
class PatternMatcher:
    def detect_all(
        self, frame: ParrotFrame, detection_functions: list[Callable]
    ) -> bool:
...
class NoisePattern(PatternMatcher):
    ...
    def __init__(
        ...
        name: str,
        labels: list[str],
        match_pattern: MatchPatternCallable,
        lowest_power_thresholds: list[float],
        throttles: dict[str, float],
        detection_after: float,
        graceperiod_length: float,
    ...
    def get_current_lowest_power_threshold(self, time: float) -> float:
        ...
        return (
            ...
            if time < self.timestamps.graceperiod_until
            else self.lowest_power_thresholds[0]
        )
...
class PatternBuilder:
    ...
    def build(self, name: str, pattern: dict) -> Optional[NoisePattern]:
        ...
        detection_calls = self.generate_matching_functions(
            pattern["threshold"], pattern["sounds"]
        )
        ...
        if "grace_threshold" in pattern:
            ...
            graceperiod_detection_calls = self.generate_matching_functions(
                pattern["grace_threshold"], pattern["sounds"]
            )
        else:
        ...
        def match_pattern(self, frame: ParrotFrame, graceperiod_until: float):
            return (
                self.detect_all(frame, graceperiod_detection_calls)
                if frame.ts < graceperiod_until
                else self.detect_all(frame, detection_calls)
            )
    ...
    def generate_matching_functions(self, thresholds, sounds):
        ...
        if ">ratio" in thresholds and len(sounds) > 1:
            detection_calls.append(
                lambda self, frame, threshold=thresholds[">ratio"], sounds=sounds: (
                    frame.classes[sounds[0]] / frame.classes[sounds[1]] >= threshold
                )
            )
        if ">f0" in thresholds:
            detection_calls.append(
                lambda self, frame, threshold=thresholds[">f0"]: frame.f0 >= threshold
            )
        if ">f1" in thresholds:
            detection_calls.append(
                lambda self, frame, threshold=thresholds[">f1"]: frame.f1 >= threshold
            )
        if ">f2" in thresholds:
            detection_calls.append(
                lambda self, frame, threshold=thresholds[">f2"]: frame.f2 >= threshold
            )
...
class Delegate(ParrotDelegate):
    ...
    def set_class_names(self, classes: set[str]) -> None:
        ...
        for name in invalid_patterns:
            logging.warning(
                f"[parrot] pattern {repr(name)} contains invalid labels and will not be used."
            )
        if invalid_patterns:
    ...
    def calculate_silence_threshold(self) -> float:
        ...
        for pattern in self.patterns.values():
            if pattern.is_active(current_time):
                thresholds.append(
                    pattern.get_current_lowest_power_threshold(current_time)
                )
    ...
    def pattern_match(self, frame: ParrotFrame) -> set[str]:
        if self.debug:
            ...
            events.write(
                "parrot",
                f"predict {winner_label} {winner_prob * 100:.2f}% pow={frame.power:.2f} f0={frame.f0:.3f} f1={frame.f1:.3f} f2={frame.f2:.3f}",
            )
